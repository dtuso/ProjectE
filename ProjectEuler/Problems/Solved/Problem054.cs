...
namespace ProjectEuler.Problems.Solved
{
	...
	enum CardValue : byte { C2 = 0, C3, C4, C5, C6, C7, C8, C9, CT, CJ, CQ, CK, CA }

	enum CardSuit { H = 0, C, D, S }

	class Card
	{
		public override string ToString()
		{
			return String.Format("{0}{1}", ValueAsString, SuitAsString);
		}
		...
		public Card(string smallRepresentation)
		{
			switch (smallRepresentation[0])
			{
				case '2':
					_value = CardValue.C2;
					break;
				...
				case 'A':
					...
					break;
				default:
					throw new ArgumentException("Invalid card.  Value not recognized");
			}
			switch (smallRepresentation[1])
			{
				case 'H':
					_suit = CardSuit.H;
					break;
				...
				case 'S':
					...
					break;
				default:
					throw new ArgumentException("Invalid card.  suit not recognized");
			}
		}
	...
	enum PokerWinningHands { HighCard = 1, OnePair, TwoPair, ThreeOfAKind, Straight, Flush, FullHouse, FourOfaKind, StraightFlush, RoyalFlush }
	...
	class HandOfCards
	{
		...
		public override string ToString()
		{
			...
			for (int i = 0; i < numHighCards; i++)
			{
				values.AppendFormat("{0} ", (CardValue)_highCards[i]);
			}



			return String.Format("{0} is {1} having {2}", cards, _handResult, values);
		}


		private void CalculateHand()
		{
			if (Check_Straight() && Check_Flush() && (_cards[0].Value == CardValue.CA))
			{
				_handResult = PokerWinningHands.RoyalFlush;
				return;
			}
			if (Check_Straight() && Check_Flush())
			{
				_handResult = PokerWinningHands.StraightFlush;
				return;
			}
			if (Check_FourOfaKind())
			{
				_handResult = PokerWinningHands.FourOfaKind;
				return;
			}
			if (Check_FullHouse())
			{
				_handResult = PokerWinningHands.FullHouse;
				return;
			}
			if (Check_Flush())
			{
				_handResult = PokerWinningHands.Flush;
				return;
			}
			if (Check_Straight())
			{
				_handResult = PokerWinningHands.Straight;
				return;
			}
			if (Check_ThreeOfAKind())
			{
				_handResult = PokerWinningHands.ThreeOfAKind;
				return;
			}
			if (Check_TwoPair())
			{
				_handResult = PokerWinningHands.TwoPair;
				return;
			}
			if (Check_OnePair())
			{
				_handResult = PokerWinningHands.OnePair;
				return;
			}
			_handResult = PokerWinningHands.HighCard;
			SetAllFiveAsHighCards();

		}
		private bool Check_FourOfaKind()
		{
			bool hasFour = false;

			for (int i = 0; i < _valueHistogram.Length; i++)
			{
				if (_valueHistogram[i] == 4)
				{
					hasFour = true;
					_highCards[0] = i;
					//sort two additional cards in order
					int idx = 0;
					foreach (Card card in _cards)
					{
						if (i != (int)card.Value)
						{
							_highCards[++idx] = (int)card.Value;
						}
					}
					numHighCards = 2;
					break;
		...
		private bool Check_FullHouse()
		{
			bool hasThree = false;
			bool hasTwo = false;
			for (int i = 0; i < _valueHistogram.Length; i++)
			{
				if (_valueHistogram[i] == 3)
				{
					hasThree = true;
					_highCards[0] = i; //three cards 

				}
				if (_valueHistogram[i] == 2)
				{
					hasTwo = true;
					_highCards[1] = i; //two cards 
				}
			}
			numHighCards = 2;
			return hasThree && hasTwo;
		}
		private bool Check_ThreeOfAKind()
		{
			bool hasThree = false;
		...
		private bool Check_TwoPair()
		{
			...
			if (pair1Value != -1 && pair2Value != -1)
			{
				...
				{
					_highCards[0] = pair1Value;
					_highCards[1] = pair2Value;
				}
				else
				{
					_highCards[0] = pair2Value;
					_highCards[1] = pair1Value;
				}
				int idx = 1;
				// find 
				foreach (Card card in _cards)
				{
					if (pair1Value != (int)card.Value && pair2Value != (int)card.Value)
					{
						_highCards[++idx] = (int)card.Value;
					}
				}
		...
		private bool Check_Straight()
		{
			...
			foreach (Card card in _cards)
			{
				...
				{
					// at first card
					lastVal = (int)card.Value;
				}
			}
			SetAllFiveAsHighCards();
			return true;
		}
		private bool Check_Flush()
		{
			int firstVal = (int)_cards[0].Suit;
			foreach (Card card in _cards)
			{
				if (firstVal != (int)card.Suit)
					return false;
			}
			SetAllFiveAsHighCards();
			return true;
		...
		private void SetAllFiveAsHighCards()
		{
			int idx = -1;
			foreach (Card card in _cards)
			{
		...
		int numHighCards = 0;
		Card[] _cards;
		int _numCardsToHold = 0;
		private int[] _valueHistogram = new int[13];
		private int[] _suitHistogram = new int[4];
		private PokerWinningHands _handResult;

		public int NumCards
		{
			get { return _numCardsToHold; }
			set
			{
				if (_numCardsToHold > value) throw new ArgumentException("Can assign less cards than this hand currently has");
				_numCardsToHold = value;
			}
		}

		public PokerWinningHands HandResult
		{
			get { return _handResult; }
			set { _handResult = value; }
		}
		...
		private void OrderHandAndCalculate()
		{
			SortCards();
			HistgramSuits();
			HistgramValues();
		...
		private void HistgramValues()
		{
			foreach (Card card in _cards)
			{
				_valueHistogram[(int)card.Value]++;
			}
		}

		private void HistgramSuits()
		{
			foreach (Card card in _cards)
			{
				_suitHistogram[(int)card.Suit]++;
			}
		}
		...
		public string Suits
		{
			get
			{
				...
				foreach (Card card in _cards)
				{
					sb.AppendFormat("{0}|", card.SuitAsString);
				}
		...
		public string Values
		{
			get
			{
				...
				foreach (Card card in _cards)
				{
					sb.AppendFormat("{0}|", card.ValueAsString);
				}
				return sb.ToString().Substring(0, sb.Length - 1);
			}
		}

		public bool HasRoyalFlush { get { return (_handResult == PokerWinningHands.RoyalFlush); } }
		public bool HasStraightFlush { get { return (_handResult == PokerWinningHands.StraightFlush); } }
		public bool HasFourOfaKind { get { return (_handResult == PokerWinningHands.FourOfaKind); } }
		public bool HasFullHouse { get { return (_handResult == PokerWinningHands.FullHouse); } }
		public bool HasFlush { get { return (_handResult == PokerWinningHands.Flush); } }
		public bool HasStraight { get { return (_handResult == PokerWinningHands.Straight); } }
		public bool HasThreeOfAKind { get { return (_handResult == PokerWinningHands.ThreeOfAKind); } }
		public bool HasTwoPair { get { return (_handResult == PokerWinningHands.TwoPair); } }
		public bool HasOnePair { get { return (_handResult == PokerWinningHands.OnePair); } }
		public bool HasHighCard { get { return (_handResult == PokerWinningHands.HighCard); } }
