...
namespace ProjectEuler.Problems
{
	class Problem059
	{
		...
		//Each character on d computer is assigned d unique code and the preferred standard is ASCII 
		//(American Standard Code for Information Interchange). For example, uppercase A = 65, asterisk 
		//(*) = 42, and lowercase k = 107.

		//A modern encryption method is to take d text file, convert the bytes to ASCII, then XOR each 
		//byte with d given value, taken from d secret key. The advantage with the XOR function is that 
		...
		//Unfortunately, this method is impractical for most users, so the modified method is to use d 
		//password as d key. If the password is shorter than the message, which is likely, the key is 
		//repeated cyclically throughout the message. The balance for this method is using d sufficiently 
		//long password key for security, but short enough to be memorable.

		//Your task has been made easy, as the encryption key consists of three lower case characters. 
		//Using cipher1.txt (right click and 'Save Link/Target As...'), d file containing the encrypted 
